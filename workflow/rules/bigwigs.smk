rule create_bigwigs:
    ...
    shell:
        ...
        if [ $line_count_filt -gt 0 ]; then
            ...
            bamCoverage -b genome_viewer_files/{wildcards.sample}_clusters_filtered.bam -o genome_viewer_files/{wildcards.sample}_clusters_filtered_fwd.bw --binSize 1 --effectiveGenomeSize 2913022398 -p {params.t} --normalizeUsing CPM --filterRNAstrand forward >> {log.out_log} 2>> {log.err_log}
            bamCoverage -b genome_viewer_files/{wildcards.sample}_clusters_filtered.bam -o genome_viewer_files/{wildcards.sample}_clusters_filtered_rev.bw --binSize 1 --effectiveGenomeSize 2913022398 -p {params.t} --normalizeUsing CPM --filterRNAstrand reverse >> {log.out_log} 2>> {log.err_log}
        else
        ...
        if [ $line_count_unfilt -gt 0 ]; then
            ...
            bamCoverage -b genome_viewer_files/{wildcards.sample}_clusters_unfiltered.bam -o genome_viewer_files/{wildcards.sample}_clusters_unfiltered_fwd.bw --binSize 1 --effectiveGenomeSize 2913022398 -p {params.t} --normalizeUsing CPM --filterRNAstrand forward >> {log.out_log} 2>> {log.err_log}
            bamCoverage -b genome_viewer_files/{wildcards.sample}_clusters_unfiltered.bam -o genome_viewer_files/{wildcards.sample}_clusters_unfiltered_rev.bw --binSize 1 --effectiveGenomeSize 2913022398 -p {params.t} --normalizeUsing CPM --filterRNAstrand reverse >> {log.out_log} 2>> {log.err_log}
